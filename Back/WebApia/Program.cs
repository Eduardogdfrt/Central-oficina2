...
public class Program
{
    public static void Main(string[] args)
    {
        ...
        // String de conexão injetada diretamente no código
        ...
        // Endpoint opcional para testar a conexão com o banco de dados
        app.MapGet("/test-db", async (SqlServerDbContext dbContext) =>
        {
            try
            {
                await dbContext.Database.CanConnectAsync();
                return Results.Ok("Conexão com o banco de dados funcionando!");
            }
            catch (Exception ex)
            {
                return Results.Problem($"Falha na conexão: {ex.Message}");
            }
        });
